...
var Command = &commands.YAGCommand{
	...
	RunFunc: func(data *dcmd.Data) (interface{}, error) {
		...
		embed := &discordgo.MessageEmbed{
			...
			Description: fmt.Sprintf("%s TV: %s\n Scheduled for: %s\n %s %s - %s %s (%s %s)", output.Team.NextEvent[0].Competitions[0].Notes[0].Headline, output.Team.NextEvent[0].Competitions[0].Broadcasts[0].Media.ShortName, output.Team.NextEvent[0].Competitions[0].Status.Type.ShortDetail, score.Competitions[0].Competitors[0].Team.Name, score.Competitions[0].Competitors[0].Score, score.Competitions[0].Competitors[1].Team.Name, score.Competitions[0].Competitors[1].Score, score.Competitions[0].Status[0].Displayclock, score.Competitions[0].Status[0].Period),
			Color:       int(color),
